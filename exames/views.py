...
def cancelar_pedido(request, pedido_id):
    ...
    if not pedido.usuario == request.user:
        pedido.save(messages.add_message(request, constants.ERROR, 'Esse pedido não é seu, portanto você não pode cancelar.'))
        return redirect('/exames/gerenciar_pedidos/')
    pedido.agendado = False
    pedido.save(messages.add_message(request, constants.SUCCESS, 'Pedido cancelado com sucesso.'))
    return redirect('/exames/gerenciar_pedidos/')
...
def acesso_medico(request, token):
    ...
    pedidos = PedidosExames.objects.filter(usuario=acesso_medico.usuario).filter(data__gte = acesso_medico.data_exames_finais).filter(data__lte = acesso_medico.data_exames_finais)
